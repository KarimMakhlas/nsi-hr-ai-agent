...
def test_web_agent_uses_the_router_search_query_instead_of_raw_question(monkeypatch):
    search_queries = []

    def fake_mcp_tool(tool_name: str, arguments: dict | None = None):
        if tool_name == "get_hr_kpi_summary":
            return {"period": "T3 2024", "source": "Excel KPI RH Q3 2024"}
        if tool_name == "web_search":
    ...
    monkeypatch.setattr(graph, "call_mcp_tool_sync", fake_mcp_tool)
    monkeypatch.setattr(graph, "generate_web_answer", lambda **_: "Réponse web prête.")

    state = base_state(
        "Compare nos KPI aux tendances du recrutement IA/Data en France",
        route="web_agent",
    )
    state["search_query"] = "benchmark KPI recrutement IA Data France 2024 2025"

    graph.web_agent(state)

    assert search_queries == ["benchmark KPI recrutement IA Data France 2024 2025"]
